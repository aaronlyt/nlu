...
def end_of_chunk(prev_tag, tag, prev_type, type_):
    ...
    checks if a chunk ended at the previous word
    Args:
...
def classification_report(y_true, y_pred, digits=2, suffix=False):
    ...
    for type_name, true_entities in d1.items():
        ...
        p = nb_correct / nb_pred if nb_pred > 0 else (0 + 1e-6)
        r = nb_correct / nb_true if nb_true > 0 else (0 + 1e-6)
        f1 = 2 * p * r / (p + r) if p + r > 0 else (0 + 1e-6)
